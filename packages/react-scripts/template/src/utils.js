...
const utils = {
  ...
  getIdentifiersToDisplay(patient, locations, currentLocation) {
    ...
    const hasCCCIdentifier = patientUtil.getIdentifiersAndPreferred(
      patient,
      IDENTIFIER_TYPES.CCC_IDENTIFIER_TYPE
    );
    const hasHCCIdentifier = patientUtil.getIdentifiersAndPreferred(
      patient,
      IDENTIFIER_TYPES.HCC_IDENTIFIER_TYPE
    );
    const hasARTIdentifier = patientUtil.getIdentifiersAndPreferred(
      patient,
    ...
    const currentLocationPrefix = utils.getCurrentLocationPrefix(
      locations,
      currentLocation
    );
    let identifiers = [],
    ...
    if (hasCCCIdentifier.length === 1) {
      ...
      const getCccCurrentLocation = hasCCCIdentifier.find(identifier =>
        identifier.identifier.match(currentLocationPrefix)
      );
      if (getCccCurrentLocation && currentLocationPrefix) {
        ...
        const getCccPreferred = hasCCCIdentifier.find(
          identifier => identifier.preferred === true
        );
        if (getCccPreferred) {
    ...
    if (hasARTIdentifier.length === 1) {
      ...
      hasARTNumber = true;
      const getArtCurrentLocation = hasARTIdentifier.find(identifier =>
        identifier.identifier.match(currentLocationPrefix)
      );
      if (getArtCurrentLocation && currentLocationPrefix) {
        identifiers.push(getArtCurrentLocation.identifier);
      } else {
        const getArtPreferred = hasARTIdentifier.find(
          identifier => identifier.preferred === true
        );
        if (getArtPreferred) {
    ...
    if (!hasARTNumber) {
      if (hasHCCIdentifier.length === 1) {
        ...
        const getHccCurrentLocation = hasHCCIdentifier.find(identifier =>
          identifier.identifier.match(currentLocationPrefix)
        );
        if (getHccCurrentLocation && currentLocationPrefix) {
          identifiers.push(getHccCurrentLocation.identifier);
        } else {
          const getHccPreferred = hasHCCIdentifier.find(
            identifier => identifier.preferred === true
          );
          if (getHccPreferred) {
    ...
    if (!identifiers.length) {
      const currentLocationPatientIdentifier = baseIdentifiers.find(
        identifier => identifier.match(currentLocationPrefix)
      );
  ...
  getPatientCheckedInEncounter: patient => {
    ...
    if (
      typeof patient.visit !== 'undefined' &&
      typeof patient.visit.encounters !== 'undefined'
    ) {
      //filter by CheckIn encounter
      let checkedInEncounters = patient.visit.encounters.filter(
        encounter =>
          encounter.encounterType.uuid ===
          ENCOUNTER_TYPES.CheckInEncounterType.uuid
      );
  ...
  getPatientCheckedInTime: patient => {
    return utils.getPatientCheckedInEncounter(patient) !== null
      ? utils.formatTime(
          utils.getPatientCheckedInEncounter(patient).encounterDatetime
        )
      : null;
  },
  ...
  hasAlert: (alerts, categories) => {
    if (typeof alerts === 'undefined' || alerts === null || alerts.length < 1) {
      return false;
    }
    let retValue = false;
    ...
    for (let i = 0; i < alerts.length; i++) {
      ...
      retValue = alertCategories.every(alertCategory =>
        categories.every(
          category =>
            alertCategories.includes(category) &&
            categories.includes(alertCategory)
        )
      );
  ...
  getAdherenceSessionNumber: obs => {
    ...
    obs.forEach(function(observation) {
      if (
        ...
        CONCEPTS.ADHERENCE_COUNSELING.AdherenceSession.uuid
      ) {
        switch (observation.value.uuid) {
          case CONCEPTS.ADHERENCE_COUNSELING.AdherenceSession.FirstSession.uuid:
            sessionNumber =
              CONCEPTS.ADHERENCE_COUNSELING.AdherenceSession.FirstSession
                .display;
            break;
          case CONCEPTS.ADHERENCE_COUNSELING.AdherenceSession.SecondSession
            .uuid:
            sessionNumber =
              CONCEPTS.ADHERENCE_COUNSELING.AdherenceSession.SecondSession
                .display;
            break;
          case CONCEPTS.ADHERENCE_COUNSELING.AdherenceSession.ThirdSession.uuid:
            sessionNumber =
              CONCEPTS.ADHERENCE_COUNSELING.AdherenceSession.ThirdSession
                .display;
            break;
          default:
            break;
  ...
  getAdherenceCounselor: obs => {
    ...
    obs.forEach(function(observation) {
      if (
        observation.concept.uuid ===
        CONCEPTS.ADHERENCE_COUNSELING.NameOfCounselor.uuid
      ) {
  ...
  getPatientCheckOutTime: patient => {
    ...
    if (
      typeof patient.visit !== 'undefined' &&
      patient.visit.stopDatetime !== null
    ) {
  ...
  calculateBMI: (weight, height) => {
    let bmi = null;
    if (weight && height) {
      // would be better to test against null here, but it appears that in most cases we pass in empty string if null, which is what we get passed through
  ...
  calculateBMIAlert: bmi => {
    ...
    if (bmi < CONCEPTS.BMI.lowCritical) {
      ...
      bmi >= CONCEPTS.BMI.lowCritical &&
      bmi <= CONCEPTS.BMI.lowNormal
    ) {
  ...
  calculateMalnutritionLevel: (bmi, muac, age, pregnant) => {
    ...
    if (
      ...
      bmi !== null &&
      age > 18 &&
      (typeof pregnant === 'undefined' ||
        (typeof pregnant !== 'undefined' && pregnant === CONCEPTS.False.uuid))
    ) {
