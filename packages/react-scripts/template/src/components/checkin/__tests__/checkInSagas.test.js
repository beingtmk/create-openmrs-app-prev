...
describe('check-in sagas', () => {
  ...
  it('check-in should call create visit and return success action when elements passed as objects', () => {
    ...
    expect(sagaTester.getCalledActions()).not.toContainEqual(
      checkInActions.checkInFailed('Unable to Submit')
    );
  });
  ...
  it('check-in should fail if invalid data passed in', () => {
    ...
    expect(sagaTester.getCalledActions()).toContainEqual(
      checkInActions.checkInFailed("Cannot read property 'uuid' of undefined")
    );
