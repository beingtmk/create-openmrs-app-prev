...
import { formActions, visitRest } from '@openmrs/react-components';
...
function* checkIn(action) {
  try {
      ...
      encounters: [
        {
          uuid: encounterUuid,
          patient: action.patient.uuid ? action.patient.uuid : action.patient,
          location: action.location.uuid
            ? action.location.uuid
            : action.location,
          encounterType: action.encounterType.uuid
            ? action.encounterType.uuid
            : action.encounterType,
          obs: action.obs,
        },
      ],
    };
    ...
    yield put(
      formActions.formSubmitSucceeded(
        'check-in',
        action.formSubmittedActionCreator
      )
    );
  } catch (e) {
